...
def main():
    ...
    Z_clean = net(X_clean)
    attn_clean = get_attn_matries(model_name, net, layer_outputs)
    Z_noise = net(X_noise)
    ...
    heads = np.arange(MODEL_NUM_HEADS[model_name])[:2]
    save_dir = f'./figures/{model_name}/attn_mat_per_noise_noise{noise_scale}'
